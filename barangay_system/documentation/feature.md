...
The system enforces a mandatory password change mechanism to ensure that default and temporary passwords are never used permanently.
This is triggered when:
*   A new account is created by an administrator (if the "Require change on first login" option is enabled), or
*   An administrator resets an existing user's password.

Enforcement behavior:
*   Upon login, the system detects the `require_password_change` flag in the user record.
*   The user is immediately redirected to the Change Password page.
*   Access to all protected dashboard pages and modules is blocked until the password is successfully updated.
*   Direct URL manipulation (bypass attempts) are intercepted by the session guard — the user is redirected back to the Change Password page.
*   After successfully setting a new password, the flag is cleared and the user proceeds normally to their dashboard.
*   The action is recorded in the Audit Log.

Self-registered residents who chose their own password during registration are not subject to this requirement.
...
*   Password changes (including mandatory first-login changes)
...
*   **Global Reports Inbox**
    ...
    *   **Advanced Filtering:** Filter by Incident Type, Processing Status, Report Source, and Severity Level.
    *   **Global Search:** Real-time search by Report ID, Location, Incident Type, Status, Severity, or Reporter Name.
    *   **Pagination:** Displays 10 records per page to optimize performance and readability.
...
*   **Analytics Monitoring**
    *   Administrators can view real-time system metrics through visual charts and ranked lists:
        *   **Overview Metrics:** Total Reports, Resolved Cases, Active/Pending, and Critical Incidents.
        *   **Incident Types (Doughnut Chart):** Breakdowns of Flood vs. Drainage issues.
        *   **Severity Distribution (Doughnut Chart):** Visualization of Low, Medium, High, and Critical alert levels.
        *   **Top Affected Areas (Ranked List):** Geographic "heatmap" ranking the most frequently reported locations.
