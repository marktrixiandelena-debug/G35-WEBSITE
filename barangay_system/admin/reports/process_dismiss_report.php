...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $statusCheck = $conn->prepare("SELECT status FROM reports WHERE id = ?");
    ...
    if ($currentReport['status'] !== 'pending') {
        header("Location: report_details.php?id=$report_id&error=" . urlencode("Only pending reports can be dismissed."));
        exit();
    }

    // --- Require a reason type ---
    if (empty($reason_type)) {
        header("Location: report_details.php?id=$report_id&error=" . urlencode("A dismissal reason is required."));
        exit();
    }
    ...
    $auditDetails = "Dismissed Report #$report_id. Reason: $reason_type.";
