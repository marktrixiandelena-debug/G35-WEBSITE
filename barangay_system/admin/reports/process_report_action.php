...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fetchStmt = $conn->prepare("SELECT status, type, location FROM reports WHERE id = ?");
    ...
    if ($db_status === 'resolved' || $db_status === 'dismissed') {
        header("Location: report_details.php?id=$report_id&error=" . urlencode("This report is finalized and cannot be modified.") . $rtParam);
        exit();
    }
    ...
    if ($status_changed) {
        ...
        if (!in_array($new_status, $allowed_next, true)) {
            header("Location: report_details.php?id=$report_id&error=" . urlencode("Invalid status transition.") . $rtParam);
            exit();
        }
    }
    ...
    $teamNameForAudit = null; // Will be set below if a team is assigned
    if ($team_id > 0 && $new_status !== 'resolved') {
        ...
        if (!$current_assignment || $current_assignment['team_id'] != $team_id) {
            ...
            $teamQ->close();
            $teamNameForAudit = $teamName; // Expose name to audit block below
    ...
    $reportLocation = $currentReport['location'] ?? '';
    $statusLabels   = ['pending' => 'Pending', 'in_progress' => 'In Progress', 'resolved' => 'Resolved'];
    if ($status_changed) {
        $fromLabel    = $statusLabels[$db_status]  ?? ucfirst($db_status);
        $toLabel      = $statusLabels[$new_status] ?? ucfirst($new_status);
        $locationNote = $reportLocation ? " at $reportLocation" : '';
        $auditDetails = "$reportType Report #$report_id status changed from $fromLabel to $toLabel$locationNote.";
        if ($teamNameForAudit && $new_status !== 'resolved') {
            $auditDetails .= " Assigned to \"$teamNameForAudit\".";
        }
    } elseif ($team_id > 0 && $teamNameForAudit) {
        $auditDetails = "$reportType Report #$report_id: Response team updated to \"$teamNameForAudit\".";
    } elseif ($team_id > 0) {
        $auditDetails = "$reportType Report #$report_id: Response team assignment updated.";
    } else {
        $auditDetails = "$reportType Report #$report_id: Case note added by admin.";
    }
