...
// trim() removes extra spaces from valid input.
$full_name = trim($_POST['full_name']);
$username = trim($_POST['username']);
$role = $_POST['role'];
$status = $_POST['status']; // Get status
$contact_number = trim($_POST['contact_number'] ?? ''); // Optional
$address = trim($_POST['address'] ?? ''); // Optional
$require_password_change = isset($_POST['require_change']) ? 1 : 0;
...
if ($stmt->execute()) {
    // Log Action
    logActivity($conn, $_SESSION['user_id'], 'Create User', "Created new user: $username ($role)");
