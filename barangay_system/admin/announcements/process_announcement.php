...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === 'create') {
        ...
        if ($stmt->execute()) {
            ...
            logActivity($conn, $_SESSION['user_id'], 'Create Announcement', "Published new $type announcement: \"$title\"");
    ...
    // -----------------------------------------------------------------------
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                logActivity($conn, $_SESSION['user_id'], 'Update Announcement', "Updated $type announcement: \"$title\"");
    ...
    // -----------------------------------------------------------------------
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            logActivity($conn, $_SESSION['user_id'], 'Delete Announcement', "Removed announcement: \"" . $titleRow['title'] . "\"");
