...
if (isset($_POST['toggle_status'])) {
    ...
    if ($new_status === 'Inactive' && $isActive > 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Fetch team name for a readable audit entry
            $tnQ = $conn->prepare("SELECT team_name FROM response_teams WHERE id = ?");
            $tnQ->bind_param("i", $team_id);
            $tnQ->execute();
            $tnRow = $tnQ->get_result()->fetch_assoc();
            $tnQ->close();
            $logTeamName  = $tnRow['team_name'] ?? "Team #$team_id";
            $statusVerb   = ($new_status === 'Active') ? 'activated' : 'deactivated';
            logActivity($conn, $_SESSION['user_id'], 'Toggle Team Status', "Response team \"$logTeamName\" has been $statusVerb");
